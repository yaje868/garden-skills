...
专业演讲设计研究发现（Nancy Duarte *Resonate*; Echo Rivera）：
观众注意力保持的关键不是"先讲完问题再讲方案"，而是交替制造张力。

但如果只会"交替"就变成套模板——不同类型有不同节奏。

**操作流程：**

写完后给每步标上 WI（What Is — 现状/问题/痛点）或 WCB（What Could Be — 方案/亮点/成果）。

然后判断你的文章属于哪种类型，按对应规则检查：

| 类型 | 自检项 | 结尾三要素 | 密度曲线 |
|---|---|---|---|---|
| **技术深挖 / 案例研究** | WI 和 WCB 要交替避免连续 3+ 步同类。技术对比内容打散穿插 | CTA 足够。回味瞬间可加可不加 | 开口短（≤3 句），中段正常，收尾留减速带 |
| **教程 / 操作指南** | 开头要有 WCB 钩子让观众看到终点。进入主体后以 WCB 为主，不强制交替 | 清晰的"下一步做什么"。不需要回味 | 开头的成果预览最密，主体展开可稍稀疏 |
| **产品测评 / 对比** | 优点和缺点交替出现。结尾落在平衡结论上，不必强行 WCB | **需要回味**，观众要带走一个判断。CTA 辅助 | 开头挂数值最密，中间对比交替，结尾放慢 |
| **新闻 / 资讯** | 天然适合交替。重点检查开头钩子够不够炸 | 信息给完即可。不需要回味，不需要 CTA | 开口最炸，后面渐疏 |
| **故事 / 叙事** | 检查高潮后是否有足够的 WCB 释放让观众回味 | **必须回味**，没有回味故事就不完整 | 开头平缓铺垫，高潮最密，结尾留白扩散 |

如果不属于以上类型：标记 WI/WCB 后看是否有 ≥ 5 步连续同类内容 → 考虑打散。

**根本原则：不要连着讲太久的同类信息。节奏来自差异，不是来自规则。**

---

### 10. 一步一屏校验

专业演示设计（Garr Reynolds *Presentation Zen*; Nancy Duarte *Slide:ology*; TED 官方演讲指南）的核心铁律：**一步只讲一个想法**。画面和声音必须对准同一件事。

口播在讲第 2 个想法时，观众的视线还停在上一屏的画面——**视听错位**是观众走神最快的原因。

**定义：**

- 「一个想法」= 可以在屏幕上用一个标题 + 一组（数字 / 代码 / 图示）讲明白的信息单元
- 「一个想法」≠ 一句话。3 句围绕同一冲击点的句子仍然是 1 个想法（如"358 条规则… 5000 个仓库… 100 万个问题" — 都在讲 scale）
- 列表列举是 1 个想法（如"四种状态：@Deprecated / @Public / @Experimental / internal"），不需要每条拆成一步

**判断标准：**

写完每步口播稿后，问自己：**「这一步的画面是什么？」**

如果答案是"先讲 X，再讲 Y，再讲 Z"（多个画面）→ 拆成多步。如果答案是"一张图 / 一段代码 / 一个数字 + 一句话" → 可以。

**常见的需要拆的情况：**

- 一步里讲了 2+ 个不同的概念（如"插件的工作机制 + 两种用法"）
- 一步里包含"机制描述 + 配置方法 + 效果"三个环节
- 一步里既有"现有数据"又有"未来计划"
...
- 画面只有一个焦点
- 口播推进到新想法 → 画面自然切到新屏
- 不出现"口播在讲 B，画面还停在 A 的细节上"的错位

---

### 11. 观众情绪映射（辅助自检工具）

> **来源说明**：本原则从多个领域的实践综合而来——Nancy Duarte *Resonate* (Sparkline 的 WI/WCB 两种状态)、认知负荷理论 (连续同类认知状态导致疲劳)、Blake Snyder *Save the Cat* (剧本 beat sheet 每 scene 标注功能标签)、Echo Rivera 的演示设计 (每 3-4 分钟需要脑力休息点)。没有单一来源要求"每步标情绪"——这是一个推理延伸，不是铁律。

**写完后给每步标一个预期观众反应**（3-5 字概括），目的是发现节奏盲区。
...
- 连续 3+ 步反应类型相同 → 节奏需要调整
- 特别警惕"吸收"状态连续超过 3 步 → 观众认知负荷过载，需要插入一个情绪释放点（震撼/放松/行动）
- 如果一部稿子超过一半的步都在"吸收" → 整稿信息密度过高，需要降低

**与 Sparkline 的关系：**

WI/WCB 是粗粒度的叙事节奏（问题 vs 方案），情绪映射是细粒度的认知节奏。
两者一起用效果最好：先迭代 WI/WCB 结构，再用情绪映射微调。
...
技术口播有自己的一套"万能词"——不是 AI 腔调，是行业陈词。观众一听就走神，因为太抽象了，脑中没画面。

**常见技术戒词：**

| ❌ 戒词 | 为什么烦人 | ✓ 改法 |
|---|---|---|
| 架构 / 范式 / 重构 | 具体指什么？ | "整个项目的分层方式" / "从头写了一遍" |
| 赋能 / 闭环 / 沉淀 | 放在任何句子里都对，但什么都没说 | 直接删，或换成具体动作："帮他发现了问题" / "整理成规则库" |
| 维度 / 视角 / 颗粒度 | 万能名词，观众不需要知道你在用哪个词 | "从另一个角度看" / "这条规则只针对这个模块" |
| 降维打击 / 抓手 / 底层逻辑 | 互联网黑话 + 过时网络梗的混合体 | 删掉。用具体对比替代 |
| 扎实 / 坚实 / 深厚 | 空洞的正面形容词 | 用数据或案例替代："跑了 5000 个仓库都没出事" |

**判断标准：这句话去掉这些词，信息量变了吗？没变 → 删。变了 → 用具体描述替换。**
...
- [ ] 没"首先 / 其次 / 最后"结构词？
- [ ] 都是具体例子，不是抽象描述？
- [ ] Sparkline 节奏自检：每步标了 WI/WCB？对照类型表（技术/教程/测评/新闻/故事）按对应规则检查了？
- [ ] 一步一屏：每步问"这一步的画面是什么？"只有 1 个画面？涉及 2+ 概念/机制+配置+效果/数据+未来 → 拆
- [ ] 情绪映射：每步标了预期观众反应？连续 3+ 步相同？超过一半在"吸收"？

### 3. 风骨层自检（去 AI 味六类 + 技术戒词）—— **最重要**
...
- [ ] 全文是否有用 "恰恰 / 反而 / 正是" 包装的句子？**去掉这些词
      意思变了吗**？没变就删。
- [ ] 是否有 "架构 / 范式 / 赋能 / 闭环 / 沉淀 / 维度 / 重构 / 降维打击"
      等技术戒词？去掉后信息量不变 → 删。用具体描述替换。
